...
g-tx-fifo-size = <256 256 64 64 64 32 32>;
dr_mode = "otg";
```

Узел использует SoC-specific compatible `sophgo,cv1800b-usb`. Драйвер dwc2 при этом накладывает параметры для cv1800/sg2002 (UTMI 16-bit PHY, AHB burst INCR16, без HNP/SRP, gadget-DMA выключен). Тип PHY и размеры берутся драйвером, отдельного `phy_type` в биндинге нет. Из клоков контроллера dwc2 включает по имени только `otg`, поэтому в узле он один (`CLK_AXI4_USB`), binding допускает clocks minItems 1.

При старте в dmesg появляется строка `dwc2 ... Invalid parameter g_dma_desc=1`. Это ожидаемо и безвредно. Железо рапортует descriptor-DMA (дефолт `g_dma_desc=1`), но quirk-функция выключает gadget-DMA, и драйвер сам сбрасывает `g_dma_desc` в 0. На перечисление это не влияет, USB поднимается high-speed.
...
`CONFIG_USB_DWC2=m`, `CONFIG_USB_DWC2_DUAL_ROLE=y` (см. Makefile, цель `kernel`). Модуль `dwc2.ko` лежит в `/lib/modules/6.18.29/kernel/drivers/usb/dwc2/dwc2.ko`. Автозагрузка через udev по DT compatible match `sophgo,cv1800b-usb`.
