...
- Console-over-USB (ACM функция). Плата это `/dev/ttyACM0` на ПК, на плате это `/dev/ttyGS0`. `getty@ttyGS0` даёт login prompt без UART-конвертера.
...
Сейчас эти опции в .config не включены. Включать когда дойдём до gadget bring-up.
...
`/usr/local/sbin/setup-usb-gadget.sh`:
...
| ACM по умолчанию | systemd-юнит `usb-gadget.service` + `getty@ttyGS0.service` |
...
После reboot контроллер DWC2 останется в `otg` режиме, но без gadget-функций (по умолчанию будет работать host если подключить периферию через OTG-переходник).
