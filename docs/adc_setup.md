...
Статус: тракт подтверждён на железе на уровне драйвера, модуль грузится,
`iio:device0` появляется, `in_voltage1_raw` читается. Внешний источник
напряжения на header не подавался, поэтому шкала и множитель делителя
ниже расчётные, замером не подтверждены.
