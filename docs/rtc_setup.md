...
## Холодный старт и системное время

После полного снятия питания RTC времени не хранит, и ранний `hctosys` падает с `unable to read the hardware clock`. Системное время на старте задаёт systemd, беря максимум из текущих часов и mtime файла `/var/lib/systemd/timesync/clock`, и двигает часы только вперёд. Роль `fake-hwclock` играет именно этот файл, поэтому сам пакет в образ не входит.

Файла нет ни в поставке `systemd`, ни в tmpfiles, ни в postinst. Юнит имеет `StateDirectory=systemd/timesync` и `User=systemd-timesync`, каталог создаётся при первом старте сервиса. На свежепрошитом образе без этого файла systemd садится на вкомпилированный `TIME_EPOCH`, равный дате сборки пакета systemd, а timesyncd затем создаёт clock-файл уже с этим ложным временем. Значение защёлкивается и переносится на все последующие холодные загрузки.

Практическое следствие: до первой сверки по NTP часы стоят в прошлом, из-за чего ломаются проверка подписей apt и валидация TLS-сертификатов. ПОДТВЕРЖДЕНО на железе W 2026-07-10. Часы на старте показывали `2026-04-13 23:01:38` (mtime clock-файла, происходящий от `TIME_EPOCH` бинарника systemd 257.13-1~deb13u1 со сборкой 2026-04-13 22:38), NTP подтянулся только в 15:27:39, а `apt update` в этом окне падал с `sqv ... Not live until 2026-05-16T09:12:42Z` на подписи InRelease.

Лечится в `Makefile`. Clock-файл кладётся в rootfs с владельцем `systemd-timesync` в таргете `rootfs-packages`, а в таргете `image` его mtime переставляется на момент сборки образа, рядом со сбросом `machine-id` и `random-seed`. Второй шаг нужен потому, что rootfs кэшируется и может быть намного старше образа. Владелец проставляется по uid из `/etc/passwd` самого rootfs, а не константой, поскольку порядок создания системных пользователей при debootstrap не гарантирован.

Диагностика на железе, если apt снова начнёт ругаться на подписи:

```sh
journalctl -b -o short-iso --no-pager | head -3     # время часов на старте
journalctl -b -o short-iso --no-pager | grep -iE 'timesync|clock|rtc'
stat -c '%y  %U:%G  %n' /var/lib/systemd/timesync/clock
timedatectl                                          # System clock synchronized: yes
```

Строка `System time advanced to timestamp on /var/lib/systemd/timesync/clock` в журнале показывает, какой именно пол времени применил systemd. Проверять успех apt надо с принудительной перекачкой подписи, иначе валидный кэш скрывает проблему:

```sh
rm -f /var/lib/apt/lists/*InRelease && apt update    # ожидаем Get:1, не Hit:1
```
