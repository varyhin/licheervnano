...
- Plate как USB host (через OTG-переходник), когда Type-C занят
...
4 Mbaud. Тактовая частота драйвера задана в `cv180x.dtsi` (узел `clk_uart0`
и далее) и составляет 25 MHz. Делитель baud разрешает любые скорости
кратные `25e6 / (16 * N)` без существенной ошибки до 1.5 Mbaud.
