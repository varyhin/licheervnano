...
Важно: cooling-устройства у нас нет (нет cpufreq/devfreq/TPU cooling-map). Поэтому `passive` и `hot` чисто информационные (видны в `/sys`, действий не вызывают), а функционально срабатывает только `critical` это аварийный `orderly_poweroff`. 110°C выбран с запасом под TPU/разгон и ниже Tj max кремния ~125°C, чтобы не ложно-срабатывать. Автоматического троттлинга нет, это отдельная задача (привязать thermal к cpufreq/devfreq cooling).
...
- `critical` trip деструктивен: при 110°C ядро инициирует `orderly_poweroff`. На пассивно охлаждаемой плате при штатной работе (включая TPU на 700 МГц) до этого далеко.
