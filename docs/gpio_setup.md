...
## Mapping gpiochip → SoC porta/b/c/p
...
| gpiochip3 | portp (PWR_GPIO) | 0x03023000 | RTC/SD1 power domain, header правая сторона |
...
| GPIOA 15 | gpiochip0 line 15 | свободен | |
...
| GPIOA 26 | gpiochip0 line 26 | свободен (EMMC_DAT2 pin) | |
...
| GPIOP 18 | gpiochip3 line 18 | свободен | |
| GPIOP 19 | gpiochip3 line 19 | I2C1 SCL (pinmux 0x2) | потеряете I2C1 |
| GPIOP 20 | gpiochip3 line 20 | I2C3 SDA (pinmux 0x2) | потеряете I2C3 |
| GPIOP 21 | gpiochip3 line 21 | I2C3 SCL (pinmux 0x2) | потеряете I2C3 |
| GPIOP 22 | gpiochip3 line 22 | I2C1 SDA (pinmux 0x2) | потеряете I2C1 |
...
Например подать high на свободную EMMC_DAT2 pin (gpiochip0 line 26):

```
# Установить high
gpioset -c gpiochip0 26=1

# Установить low
gpioset -c gpiochip0 26=0

# Прочитать
gpioget -c gpiochip0 26
```
...
```
gpioset --hold-period 60s -c gpiochip0 26=1
...
```
echo mmc0 > $LED/trigger            # вернуть дефолт вручную
```
...
`activity` (дефолт vendor у `led-user`), `cpu`, `netdev` (tx/rx/link сети), `panic` и `uleds` (`LEDS_USER`, LED из userspace через `/dev/uleds`) включены в нашем kernel-конфиге (блок scripts/config в Makefile, target kernel) и доступны в образе. `camera`-триггера в mainline нет. Дефолт у user-LED остаётся `mmc0` (индикатор SD). Любая правка `.config` требует пересборки ядра И ВСЕХ модулей (смена конфига меняет ABI структур).
...
- libgpiod 2.x в Debian 13 trixie имеет другой синтаксис чем 1.x.
  Старые howto в интернете могут указывать `gpioset gpiochip0 26=1`,
  в 2.x это работает только с `-c gpiochip0 26=1`. См. `gpioget --help`
