...
Документ собирает информацию по аудио-подсистеме SG2002 + LicheeRV Nano для bring-up под mainline Linux 6.18.29. Статус на 2026-05-29: и capture (микрофон), и playback (динамик через AW8010A) работают на железе. Голос, записанный микрофоном, слышен при `aplay -D spk` на внешнем 8Ω динамике (header VOP/VON). Усилитель включается только на время воспроизведения (стрим-гейтинг через SPK_EN, patches 0012/0013). Детали по фазам в конце документа.
...
Начиная с текущей сборки `make rootfs` (target `audio-install`) всё это уже зашито в SD-образ, вручную ничего создавать не нужно.

- `/etc/asound.conf` с устройствами `mic` (моно с микрофона через `plughw`) и `spk` (playback на DAC, `hw:0,1`).
- Пакет `alsa-utils` (`amixer`/`arecord`/`aplay`).
- Сервис `set-audio-gain.service` выставляет на boot усиление ADC `numid=1 18,0` (скрипт `/usr/local/sbin/setup-audio-gain.sh` ждёт появления `card0`).

Готовые команды на плате сразу после boot: `arecord -D mic -c 1 -r 48000 -f S16_LE /tmp/mic.wav` и `aplay -D spk /tmp/mic.wav`. Раздел ниже описывает, как собрать ту же конфигурацию вручную и как её диагностировать.
...
- `numid=1 Internal I2S Capture Volume` это аналоговый PGA, диапазон 0..24 = 0..+48 dB шагом 2 dB. Рабочий дефолт это 18 (+36 dB): чистый, слышен на динамике. `,0` ставит правый канал в минимум (при ttable правый и так не участвует).
- НЕ использовать gain 20 (`0xA800`): точечно нестабилен, даёт railing/тишину даже после CTUNE-фикса. Чистые ступени: …14, 16, 18, 22. Громкий близкий источник может клиппить на пиках это снизить до 14-16.
- Историческая заметка: до CTUNE-фикса (patch 0014) высокие ступени (≥16) рельсили из-за неподстроенной тактовой ADC, а не из-за микрофона. Патч 0014 (`RXADC_ANA3 CTUNE` по MCLK) это исправил, проверено на железе 2026-05-29: в тишине gain 10/16/18/22 дают низкий уровень (5-12%), railing ушёл.
...
for g in 18 19 20 21 22; do
  ...
  echo "=== gain=$g ==="
  arecord -D mic -c 1 -r 48000 -f S16_LE -V mono -d 3 /dev/null
done
```
...
Из vendor DTS (`/tmp/vendor-search/build/boards/default/dts/cv180x/cv180x_base.dtsi`) и TRM:
...
`/tmp/vendor-search/linux_5.10/sound/soc/cvitek/` (vendor SDK кеширован при discovery):
...
## Команды для следующих фаз

### Phase 2: DT-узлы + pinmux

```
# Изучить vendor DTS, скопировать audio nodes в cv180x.dtsi
cat /tmp/vendor-search/build/boards/default/dts/cv180x/cv180x_base.dtsi | grep -A6 'adc\|dac\|i2s_'
...
cp /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv181xadc.c \
   /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv181xdac.c \
   /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv181x_cv181xadc.c \
   /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv181x_cv181xdac.c \
   /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv1835_i2s_subsys.c \
   /tmp/vendor-search/linux_5.10/sound/soc/cvitek/cv1835_i2s.c \
...
| RXADC_CLK | `hw_params`: `SCK_DIV(4) \| DLYEN(0x19)` | `hw_params`→`setbclk_div`: SCK_DIV=bclk_div (=3), DLYEN=0x19 | то же + дописывает CTUNE в ANA3 в этой же функции |
...
| hw_params | set_mclk → CTRL1(CIC)+ANA3(CTUNE)+CLK(SCK/DLY)+DAC ANA0(ECO) | CLK(SCK/DLY)+CTRL1(IGR+CIC) | soft-reset → recommit ANA2/ANA0 → CLK+CTUNE → CTRL1 → ECO |
...
- Vendor SDK: `/tmp/vendor-search/linux_5.10/sound/soc/cvitek/` (sparse-checkout, см. memory `vendor-sdk-driver-locations`)
