...
Эта таблица речи это пример одной сессии, абсолютный уровень зависит от громкости голоса и дистанции. Воспроизводимая (не зависящая от акустики) лестница это шумовой пол в тишине, свежий замер на vendor-faithful сборке (release d77741c2, 2026-06-13):
...
- Наш рабочий драйвер это `src/linux/sound/soc/sophgo/cv1800b-sound-adc.c` = 0009 + 0015 (soft-reset) + 0016 (clock-slave; переносит enable из `trigger` в `prepare` + settle, добавляет `shutdown`, ECO-запись `0x0300A020` vendor-faithful). SDM-init, опрос RXADC_STATUS и SDM/DITHER/VCMT-макросы убраны патчем 0016. Патч 0014 (CTUNE) удалён 2026-06-01 как редундантный, драйвер CTUNE не программирует.
...
|---|---|---|---|---|
| `0x0300A020` b0 ADDI_TXDAC | «ECO», коэф. усиления аналога в DAC-блоке | `hw_params`: clear b0 (GAIN_RATIO_1) на каждый rate | не трогает | clear b0 в `hw_params` (vendor-faithful) |
...
| prepare | recommit ANA2/ANA0 → `adc_on` (CTRL0) | нет prepare | `adc_on` (CTRL0) → settle msleep(1000 мс) |
| trigger | пусто (специально) | START→enable, STOP→disable | нет (заменён на prepare/shutdown) |
| shutdown | off → `reset_adc` (b29) → `clk_off` | нет | off (CTRL0=0); reset не здесь, он в hw_params |
...
CIC=64, CTUNE=0xC (сброс-дефолт, не пишется), ECO clear b0, reset-импульс b29. После
патча 0016 различается только фаза записи (см. ниже), экспериментальные добавки убраны.
...
Чем отличается фаза (после 0016):

- enable: vendor в `prepare` + пустой `trigger`; у нас в `prepare` + settle `msleep(1000)` для floating правого входа, `trigger` нет, гашение в `shutdown` (CTRL0=0).
- soft-reset и recommit ANA0/ANA2 у нас в `hw_params`, у vendor в `shutdown`/`prepare`.

Экспериментальных добавок после 0016 не осталось: SDM-init (VCMT/DITHER/RSTSDM),
повторный enable в `trigger` и опрос RXADC_STATUS убраны, диагностика переведена
в `dev_dbg`.

Итог: после 0016 регистровый тракт байт-в-байт vendor (отличие только в фазе записи).
Слот L/R и детерминизм модулятора vendor в codec-драйвере не решает, у нас это закрыто
фиксом clock-slave (контроллер захвата стал clock slave, слот детерминирован, pick-loud
не нужен; `plughw` downmix при необходимости).

## Сверка тактовых битов I2S0-контроллера с vendor (BCLK)

Биты I2S0-контроллера (`cv1800b-tdm`), которыми правит патч 0016, сверены с
vendor-заголовком `sound/soc/cvitek/local.h` из BSP Sipeed и совпадают с
mainline-драйвером (commit 75ca8602):

- `BLK_MASTER_MODE` это bit1 регистра `BLK_MODE_SETTING` (0x00); vendor `ROLE_MASK=0x2`, `MASTER_MODE=(1<<1)`
- `BCLK_OUT_FORCE_EN` это bit6 регистра `CLK_CTRL0` (0x60); vendor `BCLK_OUT_FORCE_EN=(1<<6)`
- `BCLK_DIV` это [31:16] регистра `CLK_CTRL1` (0x64); vendor `BCLK_DIV(l)=((l<<16)&0xFFFF0000)`

Reset-дефолт `BCLK_OUT_FORCE_EN=1` подтверждён. Vendor явно сбрасывает этот
бит в `probe()` (`cv1835_i2s.c`, комментарий «blck_out output after transmission
start»), что имеет смысл только при дефолте 1. На железе очистки одного
`BLK_MASTER_MODE` не хватало, контроллер продолжал гнать BCLK, это прямое
подтверждение дефолта 1. Оговорка «точное сбросовое значение по TRM не сверено»
из преамбулы 0016 снимается, TRM не понадобился (vendor-исходник авторитетнее
для семантики регистров).
