...
FSBL в контейнер кладётся из `firmware/cv181x.bin`, это наша сборка цели `fsbl`. Рядом лежит `firmware/cv181x-vendor.bin`, готовый BL2 из SDK Sipeed. Сборка его не использует, он держится аварийным fallback-ом на случай, когда цель `fsbl` не собирается. Формат тот же, fiptool его принимает, подмена делается одним аргументом `--fsbl`. Но вместе с ним теряются патчи `patches/fsbl/0002` (гашение синего USER LED на время загрузки) и `0003` (осушение AIC8800, без него Wi-Fi не встаёт после reboot на W/WE), поэтому для готового образа fallback не подходит. Условия применения записаны в `Makefile` над целью `fip`.
