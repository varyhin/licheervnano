...
- cv181x-vendor.bin это готовый BL2 из того же SDK Sipeed, закрытый vendor-блоб.
  Сборкой не используется, держится как аварийный fallback, когда цель fsbl не
  собирается. Когда он применим и что при этом теряется, описано в Makefile
  над целью fip
