...
/* forward-port mainline: получить физ. адрес dmabuf по fd. Нужно для
 * dma-heap, где userspace не может взять физадрес (pagemap не транслирует
 * CMA-mmap). fd in, paddr out. Драйвер делает dma_buf_get + attach +
 * map_attachment + sg_dma_address. */
struct cvi_dmabuf_paddr_arg {
	int fd;
	unsigned long long paddr;
};
#define CVITPU_GET_PADDR _IOWR(IOCTL_TPU_BASE, 0x0D, unsigned long long)
