...
# "core" в узле mmc. В mainline cv180x.dtsi под этим именем стоял
# CLK_AXI4_SD0 (ветка clk_axi4, fpll/5 = 300 МГц), тогда как реальный
# источник такта карты это clk_sd0 (fpll/15 = 100 МГц, TRM таблица 8.4).
# Из-за перепутанных имён все программируемые частоты были втрое ниже
# заказанных, а отказа не было, потому что такт идентификации 133 кГц
# всё ещё попадает в допустимое окно 100-400 кГц. Исправлено патчем
# patches/linux/0023, разбор в docs/sdcard_setup.md.
#
# Как читать результат: отношение такта по регистрам к такту по мнению ядра
# должно быть 1.00x. Любое другое значение означает, что перекос базового
# такта вернулся, например при откате патча 0023 или при смене dtsi.
#
# Использование:
